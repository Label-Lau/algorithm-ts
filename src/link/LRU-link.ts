/**
 * @description LRU 缓存 - 不使用 Map
 * @author 双越老师
 */

interface IListNode {
    value: any
    key: string // 存储 key ，方便删除（否则删除时就需要遍历 this.data )
    prev?: IListNode
    next?: IListNode
}

export default class LRUCache {
    private length: number
    private data: { [key: string]: IListNode } = {}
    private dataLength: number = 0
    private listHead: IListNode | null = null
    private listTail: IListNode | null = null

    constructor(length: number) {
        if (length < 1) throw new Error('invalid length')
        this.length = length
    }

    private moveToTail(curNode: IListNode) {
        const tail = this.listTail
        if (tail === curNode) return

        // -------------- 1. 让 prevNode nextNode 断绝与 curNode 的关系 --------------
        const prevNode = curNode.prev
        const nextNode = curNode.next
        // if (prevNode) {
        //     if (nextNode) {
        //         prevNode.next = nextNode
        //     } else {
        //         debugger
        //         delete prevNode.next
        //     }
        // }
        // if (nextNode) {
        //     if (prevNode) {
        //         nextNode.prev = prevNode
        //     } else {
        //         delete nextNode.prev
        //     }

        //     if (this.listHead === curNode) this.listHead = nextNode
        // }

        if (prevNode && nextNode) {
            prevNode.next = nextNode
            nextNode.prev = prevNode
        } else if (!prevNode && nextNode) {
            delete nextNode.prev
            this.listHead = nextNode
        }

        // -------------- 2. 让 curNode 断绝与 prevNode nextNode 的关系 --------------
        delete curNode.prev
        delete curNode.next

        // -------------- 3. 在 list 末尾重新建立 curNode 的新关系 --------------
        if (tail) {
            tail.next = curNode
            curNode.prev = tail
        }
        this.listTail = curNode
    }

    private tryClean() {
        while (this.dataLength > this.length) {
            const head = this.listHead
            if (head == null) throw new Error('head is null')
            const headNext = head.next
            if (headNext == null) throw new Error('headNext is null')

            // 1. 断绝 head 和 next 的关系
            delete headNext.prev
            delete head.next

            // 2. 重新赋值 listHead
            this.listHead = headNext

            // 3. 清理 data ，重新计数
            delete this.data[head.key]
            this.dataLength = this.dataLength - 1
        }
    }

    get(key: string): any {
        const data = this.data
        const curNode = data[key]

        if (curNode == null) return null

        if (this.listTail === curNode) {
            // 本身在末尾（最新鲜的位置），直接返回 value
            return curNode.value
        }

        // curNode 移动到末尾
        this.moveToTail(curNode)

        return curNode.value
    }

    set(key: string, value: any) {
        const data = this.data
        const curNode = data[key]

        if (curNode == null) {
            // 新增数据
            const newNode: IListNode = { key, value }
            // 移动到末尾
            this.moveToTail(newNode)

            data[key] = newNode
            this.dataLength++

            if (this.dataLength === 1) this.listHead = newNode
        } else {
            // 修改现有数据
            curNode.value = value
            // 移动到末尾
            this.moveToTail(curNode)
        }

        // 尝试清理长度
        this.tryClean()
    }
}

const lruCache = new LRUCache(2)
lruCache.set('1', 1) // {1=1}
lruCache.set('2', 2) // {1=1, 2=2}
console.info(lruCache.get('1')) // 1 {2=2, 1=1}
lruCache.set('3', 3) // {1=1, 3=3}
console.info(lruCache.get('2')) // null
lruCache.set('4', 4) // {3=3, 4=4}
console.info(lruCache.get('1')) // null
console.info(lruCache.get('3')) // 3 {4=4, 3=3}
console.info(lruCache.get('4')) // 4 {3=3, 4=4}
console.info(lruCache) // 4 {3=3, 4=4}

// // 双向链表节点
// interface IListNode {
//     key: string; // 键值
//     value: any; // 值
//     prev?: IListNode; // 前驱指针
//     next?: IListNode; // 后继指针
//   }

//   export default class LRUCache {
//     private length: number; // 缓存池最大长度
//     private data: Map<string, IListNode> = new Map(); // 存储缓存数据的 Map
//     private dataLength: number = 0; // 当前缓存数据数量
//     private listHead: IListNode | null = null; // 链表头节点
//     private listTail: IListNode | null = null; // 链表尾节点

//     constructor(length: number) {
//       if (length < 1) throw new Error("invalid length");
//       this.length = length;
//     }

//     // 将节点移动到链表末尾
//     private moveToTail(curNode: IListNode) {
//       const tail = this.listTail;
//       if (tail === curNode) return;

//       const { prev, next } = curNode;

//       // 断绝 curNode 和 prev、next 的指针关系
//       if (prev) prev.next = next;
//       if (next) next.prev = prev;

//       // 新建 curNode 和 tail 的指针关系
//       curNode.prev = this.listTail;
//       curNode.next = null;
//       if (tail) tail.next = curNode;

//       // 更新链表尾部指针和链表头指针
//       this.listTail = curNode;
//       if (!this.listHead) this.listHead = curNode;
//     }

//     // 尝试清理缓存
//     private tryClean() {
//       // 计算链表应保留的最大长度
//       const maxLength = Math.floor(this.length / 2);
//       while (this.dataLength > maxLength) {
//         const head = this.listHead;
//         if (!head) throw new Error("head is null");
//         const headNext = head.next;
//         if (!headNext) throw new Error("headNext is null");

//         // 断绝 head 和 headNext 的指针关系
//         head.next = null;

//         // 更新链表头部指针
//         this.listHead = headNext;
//         this.listHead.prev = null;

//         // 删除缓存池中的数据
//         this.data.delete(head.key);
//         this.dataLength--;
//       }
//     }

//     // 获取缓存中指定键的值
//     get(key: string): any {
//       const curNode = this.data.get(key);
//       if (!curNode) return null;

//       // 检查是否需要将 curNode 移动到链表末尾
//       if (this.listTail === curNode) {
//         return curNode.value;
//       }

//       this.moveToTail(curNode);
//       return curNode.value;
//     }

//     // 设置缓存中指定键的值
//     set(key: string, value: any) {
//       const curNode = this.data.get(key);
//       if (!curNode) {
//         // 创建新节点并将其加入到链表末尾
//         const newNode: IListNode = {
//           key,
//           value,
//           prev: this.listTail,
//           next: null,
//         };

//         if (this.listTail) {
//           this.listTail.next = newNode;
//         }

//         this.listTail = newNode;
//         if (!this.listHead) this.listHead = newNode;

//         // 存储新节点到缓存池中，并更新缓存数据数量
//         this.data.set(key, newNode);
//         this.dataLength++;
//       } else {
//         // 更新已有节点的值，并将其移动到链表末尾
//         curNode.value = value;
//         this.moveToTail(curNode);
//       }

//       // 尝试清理缓存
//       this.tryClean();
//     }
//   }
